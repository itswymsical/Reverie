...
namespace Reverie.Content.Terraria.NPCs.Warden
{
	...
	public class WardenArm : ModNPC, IDrawPrimitive
	{
        ...
        private void ManageCaches()
        {
            ...
            Vector2 pos = NPC.Center + (player.DirectionTo(NPC.Center) * (Size.Length() * Main.rand.NextFloat(0.5f, 1.1f))) + (Main.rand.NextVector2Unit() * Main.rand.NextFloat(1.0f, 4.0f));
        ...
        private void CrushPlayer(Player target)
        {
            ...
            Vector2 desiredPosition = grabInitialPosition;
            ...
            int hitsRequired = Main.masterMode ? 5 : 3;
            ...
            if (AITimer > 240)
            {
                int damage = Main.masterMode ? 150 : 100;
        ...
        {
            Vector2 shakeOffset = new Vector2(Main.rand.NextFloat(-0.5f, 1f), Main.rand.NextFloat(-0.5f, 1f)) * value;
        ...
        private void PelletAttack(Player target)
        {
            ...
            float speed = MathHelper.Clamp(Vector2.Distance(NPC.Center, position), -14f, 14f);
            Move(position, speed);

            int shotDelay = NPC.life < NPC.lifeMax * 0.20f ? 60 : 90;
            ...
            if (ShotTimer >= shotDelay)
            {
                ...
                for (int i = -1; i <= 1; i++)
                {
                    Vector2 velocity = angle.RotatedBy(spreadAngle * i) * 9.5f;
