namespace Reverie.Content.NPCs.Shelledrake;
...
public class Shelledrake : ModNPC
{
    ...
    private void ChargeAttack()
    {
        ...
        float chargeProgress = (StateTimer - BACKSTEP_DURATION) / (CHARGE_DURATION - BACKSTEP_DURATION);
        float targetSpeed = CHARGE_SPEED * NPC.direction;
    ...
    private void ProjectileAttack()
    {
        if (StateTimer % PROJECTILE_INTERVAL == 0 && Main.netMode != NetmodeID.MultiplayerClient)
        {
            ...
            float angleStep = (ARC_ANGLE_MAX - ARC_ANGLE_MIN) / (PROJECTILES_PER_VOLLEY - 1);

            for (int i = 0; i < PROJECTILES_PER_VOLLEY; i++)
            {
                float angle = MathHelper.ToRadians(ARC_ANGLE_MIN + (angleStep * i));
                if (NPC.direction == -1)
                    angle += MathHelper.Pi;

                Vector2 velocity = angle.ToRotationVector2() * PROJECTILE_SPEED;
