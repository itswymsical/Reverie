...
using static Reverie.Content.NPCs.Bosses.KingSlime.KinguSlime;

namespace Reverie.Content.NPCs.Bosses.KingSlime
{
    public partial class KingSlimeRework : ModNPC // gonna make a partial class that contains all of the properties, constants, states and helper methods
    {
        #region Enum States
        ...
        private ref float SequenceStep => ref NPC.ai[2];
        private ref float ActionCounter => ref NPC.ai[3]; // Tracks repetitions within a step

        private BossPhase CurrentPhase => NPC.life <= (int)(NPC.lifeMax * 0.5f) ? BossPhase.Phase2 : BossPhase.Phase1;

        private bool IsEnraged => NPC.life <= (int)(NPC.lifeMax * 0.25f) && CurrentPhase == BossPhase.Phase2;

        private const float STROLL_SHORT = 120f;
        private const float STROLL_MEDIUM = 180f;
        private const float STROLL_NORMAL = 240f;
        private const float STROLL_LONG = 300f;
        #endregion
        ...
        private void DoHop(Player target)
        {
        ...
        private void DoGroundPound(Player target) //  this needs to be refactored to only peform GroundPound, BounceHouse needs it's own AI method.
        {
            float dirToPlayer = Math.Sign(target.Center.X - NPC.Center.X);
        ...
        private void DoTeleport(Player target)
        {
            switch (teleportPhase)
            {
                case TeleportPhase.Prepare:
                    ...
                    TeleportDust(1f); // More intense dust
                    break;
                ...
                case TeleportPhase.Execute:
                    ...
                    else if (Timer <= 25f)
                    {
                        ...
                        TeleportDust(3f - fadeInProgress * 2f); // Intense dust that fades
                    }
        ...
        public override void FindFrame(int frameHeight)
        {
            ...
            else if (State == AIState.Teleport)
            {
